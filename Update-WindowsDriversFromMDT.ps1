function global:Update-WindowsDrivers([string]$MDTDeploymentSharePath) {
    $share = $MDTDeploymentSharePath
        ...
        foreach($hwid in $driver.Node.PNPId) {
            if($devices_list.Contains($hwid)) {
                if($drivers_found.ContainsKey($hwid)) {
                    ...
                    if($added_driver.CompareTo($tobe_driver) -lt 0) { #listed driver is older
    ...
    <#
    .SYNOPSIS
        This command will check CP deployment for updated drivers, and install them into Windows.
    #>
}
